 ...
 */
export function isTripartiteContextValue(members: ReadonlyArray<string>): boolean {
 ...
 */
function memberKeys(node: ESTree.Node): ReadonlyArray<string> {
	const keys: Array<string> = [];
	const addKey = (key: ESTree.Node): void => {
		if (key.type === "Identifier") keys.push(key.name);
		else if (key.type === "Literal" && isString(key.value)) keys.push(key.value);
	};
	if (node.type === "TSInterfaceDeclaration") {
		for (const member of node.body.body) {
			if (member.type === "TSPropertySignature" || member.type === "TSMethodSignature") addKey(member.key);
		}
 ...
 */
function typeName(node: ESTree.TSTypeReference): string | null {
 ...
 */
function isCreateContext(callee: ESTree.Node): boolean {
...
export const requireTripartiteContextRule = defineRule({
	meta: {
		...
		docs: {
			description:
				"Require React context value types to be the tripartite { state, actions, meta } shape; missing keys leak an implicit, unnamed I/O channel.",
		},
		messages: {
			tripartiteContext:
				"Context value type is missing required key(s): {{missing}}. A context value must be { state, actions, meta } so its I/O boundary is explicit.",
		},
	},
	createOnce(context) {
		const localTypes = new Map<string, ReadonlyArray<string>>();
		 ...
		 */
		function resolveKeys(typeArg: ESTree.Node): ReadonlyArray<string> | null {
			if (typeArg.type === "TSTypeLiteral") return memberKeys(typeArg);
		...
		return {
			...
			CallExpression(node) {
				if (isCreateContext(node.callee)) calls.push(node);
			},
			"Program:exit"() {
				for (const call of calls) {
					const params = call.typeArguments?.params;
					if (params === undefined || params.length === 0) continue;
					const first = params[0];
					if (first === undefined) continue;
					const keys = resolveKeys(first);
					if (keys === null) continue;
					if (isTripartiteContextValue(keys)) continue;
