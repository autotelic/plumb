...
export const requireExhaustiveTagSwitchRule = defineRule({
	meta: {
		...
		docs: {
			description:
				"Require `default`-free switches over a `_tag` discriminant so exhaustiveness checking flags every site a new variant must touch.",
		},
		messages: {
			noDefault:
				"This switch matches on `_tag` but has a `default` arm, which hides unhandled variants from exhaustiveness checking. Remove it and list every variant explicitly so adding one fails type-check here.",
		},
	},
	createOnce(context) {
		return {
