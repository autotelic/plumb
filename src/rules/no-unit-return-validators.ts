...
function returnsVoid(node: ValidatorFunction): boolean {
	...
	if (type.type === "TSVoidKeyword" || type.type === "TSUndefinedKeyword") return true;
...
export const noUnitReturnValidatorsRule = defineRule({
	meta: {
		...
		docs: {
			description:
				"Disallow validators that return nothing (void/undefined); a check should preserve what it learned by returning the refined value.",
		},
		messages: {
			unitValidator:
	...
	createOnce(context) {
		const report = (name: string, node: ESTree.Node, validator: ValidatorFunction) => {
			if (!VALIDATOR_NAME.test(name)) return;
			if (!returnsVoid(validator)) return;
		...
		return {
			ExportNamedDeclaration(node) {
				...
				if (declaration.type === "FunctionDeclaration") {
					if (declaration.id !== null) report(declaration.id.name, declaration, declaration);
					return;
				}
