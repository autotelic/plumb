...
export const requireSortComparatorRule = defineRule({
	meta: {
		...
		docs: {
			description:
				"Require an explicit comparator on .sort()/.toSorted(); the default lexicographic sort makes canonical order accidental.",
		},
		messages: {
			missingComparator:
				"This sort has no comparator, so ordering defaults to string comparison and the canonical order is accidental. Pass an explicit comparator so the intended order is stated here.",
		},
	},
	createOnce(context) {
		return {
