...
export const noManualAncestorWalksRule = defineRule({
	meta: {
		...
		messages: {
			useGetAncestors:
				"Hand-rolled ancestor walking via `.parent` obscures traversal intent. Use `context.sourceCode.getAncestors(node)` (root-first array) and express the search declaratively.",
		},
	},
	createOnce(context) {
		return {
			MemberExpression(node) {
				...
				if (node.property.name !== "parent") return;
