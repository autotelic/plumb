...
export const noStackedJsdocBlocksRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Disallow stacked JSDoc blocks; tools attach only the last block to the following declaration, so any earlier block silently documents nothing.",
        },
        messages: {
            stackedJsdoc: "This JSDoc block is immediately followed by another; only the last one attaches to the declaration, so this block documents nothing. Merge the content into one block or delete the orphan.",
        },
    },
    createOnce(context) {
        return {
