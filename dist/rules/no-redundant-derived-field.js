...
function lastTypeNameSegment(typeName) {
    if (typeName.type === "Identifier")
        return typeName.name;
    if (typeName.type === "TSQualifiedName")
        return lastTypeNameSegment(typeName.right);
    return null;
}
...
function propertyAnnotation(member) {
    if (member.type !== "TSPropertySignature")
        return null;
 ...
 */
function isCollectionLike(annotation) {
 ...
 */
function arrayOfPrimitiveKeys(annotation) {
    if (annotation === null || annotation === undefined)
        return null;
    let elementType;
    if (annotation.type === "TSArrayType") {
    ...
    else if (annotation.type === "TSTypeOperator" &&
        ...
        if (name === "Array" || name === "ReadonlyArray")
            elementType = annotation.typeArguments.params[0];
    }
    if (elementType === undefined)
        return null;
    if (elementType.type === "TSStringKeyword")
        return "string";
    if (elementType.type === "TSNumberKeyword")
        return "number";
    return null;
}
 ...
 */
function keyKeywordOf(annotation) {
    if (annotation?.type !== "TSTypeReference")
        return null;
    const first = annotation.typeArguments?.params[0] ?? null;
    if (first === null || first === undefined)
        return null;
    if (first.type === "TSStringKeyword")
        return "string";
    if (first.type === "TSNumberKeyword")
        return "number";
    return null;
}
/**
...
export const noRedundantDerivedFieldRule = defineRule({
    meta: {
        type: "suggestion",
        docs: {
            description: "Disallow fields that duplicate information derivable from a sibling field on the same type (derived counts, parallel key/order arrays); compute or encode them instead.",
        },
        messages: {
            redundantCount: "`{{name}}` restates what is derivable from `{{sibling}}`, giving the type two sources of truth that can drift. Drop it and compute the count at read time.",
            parallelKeyArray: "`{{name}}` mirrors the keys of `{{sibling}}` as a second source of truth that can drift. Encode the canonical form (e.g. ordering) in the collection itself instead of a parallel array.",
        },
    },
    createOnce(context) {
        function checkMembers(members) {
            for (const member of members) {
                ...
                const sibling = members.find((other) => {
                    ...
                    if (otherName === null)
                        return false;
                    if (keyKeywordOf(propertyAnnotation(other)) !== elementKey)
                        return false;
