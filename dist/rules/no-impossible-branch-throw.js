...
const IMPOSSIBLE_MESSAGE = /(?:never\s+happen|impossible|unreachable|invariant|cannot\s+happen|can'?t\s+happen|unexpected\s+state|not\s+possible)/iu;
...
export const noImpossibleBranchThrowRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Disallow hand-asserted impossible branches (throws about never-happens/invariants); encode the case out of the type so the compiler rejects it.",
        },
        messages: {
            impossibleThrow: "This branch is asserted impossible by hand (\"{{message}}\"). If it truly cannot occur, parse/encode it out of the type (Option, parsed/refined type) so the compiler rejects it; otherwise return a structured failure.",
        },
    },
    createOnce(context) {
        return {
            ...
            ThrowStatement(node) {
                ...
                if (message === null)
                    return;
                if (!IMPOSSIBLE_MESSAGE.test(message))
                    return;
