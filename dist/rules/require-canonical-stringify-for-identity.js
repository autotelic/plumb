...
export const requireCanonicalStringifyForIdentityRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Disallow JSON.stringify as an identity function (equality comparison, Map/Set key, hash input); use a canonical key-sorted serializer instead.",
        },
        messages: {
            nonCanonicalIdentity: "`JSON.stringify` here follows property insertion order, so equal data can produce different strings and identical-looking data can hide inequality. Use a canonical serializer (recursively key-sorted) before using the result for {{use}}.",
        },
    },
    createOnce(context) {
