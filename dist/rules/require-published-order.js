 ...
 */
function isOrderingReference(type) {
    if (type.type !== "TSTypeReference")
        return false;
    const name = type.typeName;
    if (name.type === "Identifier")
        return name.name === "Ordering";
...
function returnsComparatorOrdering(returnType) {
    if (returnType === null || returnType === undefined)
        return false;
    const annotation = returnType.typeAnnotation;
    if (annotation.type === "TSUnionType")
        return annotation.types.every(isSentinelLiteral);
...
export const requirePublishedOrderRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Require modules that export comparators to publish a named Order (Order.make) so call sites compose through one total order instead of hand-rolling comparisons.",
        },
        messages: {
            missingPublishedOrder: "`{{name}}` exposes an ordering but the module publishes no named order. Export `export const Order = Order.make({{name}})` so comparisons compose through one total order.",
        },
    },
    createOnce(context) {
        return {
                ...
                for (const statement of node.body) {
                    ...
                    if (declaration?.type === "VariableDeclaration") {
                        for (const declarator of declaration.declarations) {
                            ...
                            if (name === "Order")
                                hasPublishedOrder = true;
                            if (!/^compare/u.test(name))
                                continue;
