...
export const noExportedMutableStateRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Disallow exported let/var bindings and mutation of module-scope values; keep module state immutable so construction-time invariants can't be broken.",
        },
        messages: {
            exportedBinding: "`{{name}}` is exported as a mutable binding, so importers can change it under every other consumer and break whatever invariants it carried. Export a `const`.",
            moduleMutation: "`{{name}}` lives at module scope, so this mutation is invisible shared state across every caller: the blessed-surface-area problem. Make it local, or hold it in explicit state/service passed by parameter.",
        },
    },
    createOnce(context) {
        ...
        function checkTarget(node) {
            ...
            if (name === null || !topLevelNames.has(name))
                return;
        ...
        return {
            Program(node) {
                topLevelNames = new Set(node.body.flatMap((statement) => statement.type === "VariableDeclaration"
                    ? statement.declarations.flatMap((declarator) => declarator.id.type === "Identifier" ? [declarator.id.name] : [])
