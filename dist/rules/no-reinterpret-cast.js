 ...
 */
function isSoleReturnOfAssertion(body) {
 ...
 */
function hasAdjacentSafetyComment(sourceCode, node) {
...
export const noReinterpretCastRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Disallow type reinterpretation via `as`; absence must be modeled by the type system, with SAFETY-commented exceptions.",
        },
        messages: {
            noReinterpret: "Type assertion discards type evidence. Narrow with discriminants/`in` checks or a type predicate instead.",
            noLaundering: "`{{name}}` exists only to perform a type assertion: a laundering wrapper that hides the cast from review and tooling.",
        },
    },
    createOnce(context) {
        return {
