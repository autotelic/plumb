...
const SENTINELS = new Set([-1, 0, 1]);
const ORDERING_OPERATORS = new Set([">", "<", ">=", "<="]);
function sentinelValue(expression) {
    if (expression.type === "Literal") {
        const numeric = readField(expression, "value");
...
function chainIsSentinel(expression) {
    if (sentinelValue(expression) !== null)
        return true;
...
function hasOrderingComparison(expression) {
    ...
    if (expression.type === "ConditionalExpression") {
        return (hasOrderingComparison(expression.test) ||
...
function coveredByOuterSentinelChain(node, ancestors) {
    for (let index = ancestors.length - 1; index >= 0; index--) {
        const current = ancestors[index];
        if (current.type === "ConditionalExpression") {
            if (chainIsSentinel(current))
                return true;
        }
...
function asSingleReturnSentinel(statement) {
    const target = statement.type === "BlockStatement" && statement.body.length === 1
...
function ifChainIsSentinel(node) {
    let branch = node;
    while (branch !== null) {
        if (!asSingleReturnSentinel(branch.consequent))
            return false;
...
export const noSentinelComparisonUnionRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Disallow hand-built -1/0/1 ordering results from comparison expressions; use a named Order/Ordering or a domain comparison instead.",
        },
        messages: {
            sentinelComparisonUnion: "This comparison hand-builds a -1/0/1 result. Publish a named order (Order.compare + Ordering.match) or return the compared values directly.",
        },
    },
    createOnce(context) {
        return {
            ConditionalExpression(node) {
                if (!chainIsSentinel(node))
                    return;
                if (coveredByOuterSentinelChain(node, ancestorsOf(context.sourceCode, node)))
                    return;
                if (!hasOrderingComparison(node))
                    return;
            ...
            IfStatement(node) {
                ...
                if (parent?.type === "IfStatement" && parent.alternate === node)
                    return;
                if (!ifChainIsSentinel(node))
                    return;
                const conditions = [node.test];
                let branch = node.alternate;
                while (branch !== null) {
                ...
                if (!conditions.some(hasOrderingComparison))
                    return;
                context.report({ node, messageId: "sentinelComparisonUnion" });
            },
        };
