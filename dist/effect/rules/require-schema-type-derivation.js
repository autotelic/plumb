...
function isSchemaStructCall(expression) {
    ...
    if (callee.type !== "MemberExpression" || callee.computed)
        return false;
    if (callee.property.type !== "Identifier" || !STRUCT_PROPERTIES.has(callee.property.name))
        return false;
    const object = callee.object;
    return (object.type === "Identifier" && /schema$/iu.test(object.name));
}
...
export const requireSchemaTypeDerivationRule = defineRule({
    meta: {
        type: "problem",
        docs: {
            description: "Require static types paired with an Effect Schema to be derived via Schema.Type<typeof schema> instead of re-declaring the shape by hand.",
        },
        messages: {
            handWrittenTwin: "`{{name}}` re-declares the shape of `{{schema}}` by hand, giving the data two sources of truth that drift. Derive it as `Schema.Type<typeof {{schema}}>` (or rename if unrelated).",
        },
    },
    createOnce(context) {
        ...
        return {
            VariableDeclarator(node) {
                if (node.id.type !== "Identifier" || node.init === null || node.init === undefined)
                    return;
                if (!isSchemaStructCall(node.init))
                    return;
